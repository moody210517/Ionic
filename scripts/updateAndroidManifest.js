(function () {
    ...
    function writePreferences (context) {
      // read manifest
      ...
      console.log('Start custom update Android Manifest');
      
      // read manifest
      var manifest = getManifest(context)
  
      if (!manifest) {
        console.warn('Android platform is not yet ready')
        return false
      }
    ...
    function getAppLinkIntentFilterData (preferences) {
      ...
      for (var i = 0; i < linkDomains.length; i++) {
        ...
        if (linkDomain.indexOf('app.link') !== -1) {
          ...
          intentFilterData.push(getAppLinkIntentFilterDictionary(linkDomain, preferences.androidPrefix, preferences.androidScheme))
        }
      }
    ...
    function getAppLinkIntentFilterDictionary (linkDomain, androidPrefix, androidScheme) {
        var scheme = 'https'
        var output = {
            '$': {
        ...
        if (androidPrefix) {
          /**
           * My custom update
           * Force pathPrefix to '/' and scheme to 'http'
           */
            output['$']['android:pathPrefix'] = '/'
            output['$']['android:scheme'] = androidScheme || 'http'
